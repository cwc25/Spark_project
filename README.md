...
* 第一个方案：聚合源数据 （把相同key的记录变成一条，value为每个记录的拼成的字符串）
...
* 第二个方案：过滤导致倾斜的key （直接舍弃掉导致数据倾斜的key)
...
第三个方案，提高shuffle操作的reduce并行度 (reduce加并行度参数）
...
### 使用随机key实现双重聚合 （大的key比如1，变成2_1, 1_1拆成了2部分）分别计算在map去掉2_, 1_,合并成一个key继续操作。）
...
### 将reduce join转换为map join （将小的rdd broadcast到各个executor,用Mappartition(foreach嵌套foreach来计算，不用reduce来计算）
...
### sample采样倾斜key进行两次join （把导致数据倾斜的key,单独拿出来做一个rdd,单独计算）
